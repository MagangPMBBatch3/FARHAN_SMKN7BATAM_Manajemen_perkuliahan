...
async function loadKelas() {
    ...
    kelasList = data?.data?.currentDosenKelas || [];
...
async function loadPertemuanData(pageAktif = 1, pageArsip = 1) {
    ...
    showTableLoading('dataPertemuan', 10, 10);
    showTableLoading('dataPertemuanArsip', 10, 10);

    // Query Pertemuan Aktif - hanya kelas yang diampu dosen
    const queryAktif = `
    query($first: Int, $page: Int, $search: String, $kelas_id: Int, $status_pertemuan: StatusPertemuan) {
        currentDosenPertemuan(first: $first, page: $page, search: $search, kelas_id: $kelas_id, status_pertemuan: $status_pertemuan) {
            data { 
    ...
    renderPertemuanTable(dataAktif?.data?.currentDosenPertemuan?.data || [], 'dataPertemuan', true);

    // Query Pertemuan Arsip
    const queryArsip = `
    query($first: Int, $page: Int, $search: String) {
        currentDosenPertemuanArsip(first: $first, page: $page, search: $search) {
            data { 
                id kelas_id pertemuan_ke tanggal waktu_mulai waktu_selesai 
                materi metode ruangan_id status_pertemuan link_daring catatan
                kelas { kode_kelas nama_kelas mataKuliah { nama_mk } }
    ...
    renderPertemuanTable(dataArsip?.data?.currentDosenPertemuanArsip?.data || [], 'dataPertemuanArsip', false);

    updatePagination(dataAktif?.data?.currentDosenPertemuan?.paginatorInfo, 'Aktif');
    updatePagination(dataArsip?.data?.currentDosenPertemuanArsip?.paginatorInfo, 'Arsip');
}
...
function getMetodeBadge(metode) {
    const badges = {
        ...
        'TatapMuka': '<span class="px-2 py-1 text-xs font-semibold rounded-full bg-purple-100 text-purple-800">Tatap Muka</span>',
