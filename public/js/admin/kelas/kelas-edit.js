...
async function openEditModal(id, kode, kelas, matakuliah, dosen, semester, kapasitas, kuota, status) {
...
async function updateKelas() {
    ...
    const NewKouta = document.getElementById('editKuota').value;
    ...
    if (!NewKode) return alert("Kode Kelas Tidak Boleh Kosong"); {
        const mutation = `
        mutation {
            updateKelas(id: ${id}, input:{kode_kelas: "${NewKode}", nama_kelas: "${NewKelas}", mata_kuliah_id: ${NewMataKuliahId}, dosen_id: ${NewDosen}, semester_id: ${NewSemester}, kapasitas: ${NewKapasitas}, kuota_terisi: ${NewKouta}, status: "${NewStatus}"}){
                kode_kelas
                nama_kelas
            }
        }`;
